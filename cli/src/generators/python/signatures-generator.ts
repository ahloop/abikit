...
import { PythonStructExtractor } from './struct-extractor';
...
`;
     ...
     */
    private extractTypesFromContract(contract: any, primaryType: string): Record<string, Array<{ name: string; type: string }>> | null {
        // Try to extract from contract ABI first
        if (contract) {
            const extractedTypes = PythonStructExtractor.extractStructTypes(contract, primaryType);
            if (extractedTypes) {
                return extractedTypes;
            }
        }

        // Fallback to hardcoded types for known cases
