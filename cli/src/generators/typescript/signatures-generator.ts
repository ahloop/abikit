...
export class SignaturesGenerator {
     ...
     */
    async generateSignatures(signatureItems: SignatureItem[], _graph: ContractGraph): Promise<void> {
        await this.generateDomain();
        await this.generateSignatureModules(signatureItems);
...
`;
     ...
     */
    private async generateSignatureModules(signatureItems: SignatureItem[]): Promise<void> {
        for (const item of signatureItems) {
            const fileName = `src/signatures/${this.toKebabCase(item.primaryType)}.ts`;
            const content = this.renderSignatureModule(item);
     ...
     */
    private renderSignatureModule(item: SignatureItem): string {
        const structName = item.primaryType;

        // Get the type definition from the hardcoded types
        const typeDefinition = this.getTypeDefinition(structName);
        if (!typeDefinition) {
